#!/bin/bash
# scripts/G_6_benchmark.sh <cni_name> [mode/flag]
...
if [[ $RESULTS_BASE == *"rules_"* ]]; then
    # Module 2 specific case where G_6_run_module2.sh overrides RESULTS_BASE
...
InstallCNI() {
    ...
    case $CNI in
        flannel)
            curl -sL -o G_6_kube-flannel.yml https://raw.githubusercontent.com/flannel-io/flannel/master/Documentation/G_6_kube-flannel.yml
            [ "$MODE" == "hostgw" ] && sed -i 's/"Type": "vxlan"/"Type": "host-gw"/' G_6_kube-flannel.yml
            kubectl apply -f G_6_kube-flannel.yml
        ...
        calico)
            ...
            curl -sL -o G_6_calico-cr.yaml https://raw.githubusercontent.com/projectcalico/calico/v3.27.0/manifests/custom-resources.yaml
            sed -i 's/192.168.0.0\/16/10.244.0.0\/16/g' G_6_calico-cr.yaml
            sed -i '/kind: Installation/,/spec:/ s/spec:/spec:\n  registry: quay.io/' G_6_calico-cr.yaml
            [ "$MODE" == "bgp" ] && sed -i 's/encapsulation: IPIP/encapsulation: None/' G_6_calico-cr.yaml
            kubectl apply -f G_6_calico-cr.yaml
        ...
        cilium)
            ...
            if [ "$MODE" == "native" ]; then
                bin/G_6_cilium install --wait --set routingMode=native --set autoDirectNodeRoutes=true --set ipv4NativeRoutingCIDR=10.244.0.0/16
            else
                bin/G_6_cilium install --wait
            fi
            ;;
    esac
    sleep 30
...
SetupWorkload() {
    ...
    sed "0,/kubernetes.io\/hostname:.*/s//kubernetes.io\/hostname: $SERVER_NODE/" manifests/G_6_iperf3.yaml > manifests/G_6_temp.yaml
    sed "0,/kubernetes.io\/hostname: $SERVER_NODE/! s/kubernetes.io\/hostname:.*/kubernetes.io\/hostname: $CLIENT_NODE/" manifests/G_6_temp.yaml > manifests/G_6_temp2.yaml
    ...
    sed -e 's|networkstatic/iperf3|quay.io/networkstatic/iperf3|g' \
        ...
        manifests/G_6_temp2.yaml > manifests/G_6_iperf3_patched.yaml

    kubectl apply -f manifests/G_6_iperf3_patched.yaml
...
RunBenchmark() {
    ...
    perf stat -a -e cycles,instructions,cpu-clock -o $RESULTS_DIR/G_6_perf_cycles.txt sleep 45 &
    PERF_PID=$!
    kubectl exec $CLIENT_POD -- iperf3 -c $SERVER_IP -t 45 --json > $RESULTS_DIR/G_6_iperf_tcp.json || echo "{}" > $RESULTS_DIR/G_6_iperf_tcp.json
    wait $PERF_PID || true
    sleep 5
    kubectl exec $CLIENT_POD -- iperf3 -c $SERVER_IP -u -b 100M -t 45 --json > $RESULTS_DIR/G_6_iperf_udp.json || echo "{}" > $RESULTS_DIR/G_6_iperf_udp.json
    sleep 5
    kubectl exec $CLIENT_POD -- ping -c 10 $SERVER_IP > $RESULTS_DIR/G_6_ping_latency.txt || echo "Ping failed" > $RESULTS_DIR/G_6_ping_latency.txt
    docker stats --no-stream $(docker ps --format '{{.Names}}' | grep "${CLUSTER_NAME}") > $RESULTS_DIR/G_6_cpu_stats.txt
}
