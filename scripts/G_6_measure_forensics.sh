#!/bin/bash
# scripts/G_6_measure_forensics.sh - Deep Networking Forensics Suite (Patched)
...
kubectl exec $CLIENT_POD -- mtr --report --report-cycles 10 --tcp -P 5201 $SVC_IP > $RESULTS_DIR/G_6_path_mtr.txt
...
for SIZE in 1472 1450 1440 1420 1400 1350 1300; do
    ...
    if kubectl exec $CLIENT_POD -- ping -c 1 -M do -s $SIZE $TARGET_IP > /dev/null 2>&1; then
        echo "PASS: $SIZE"
        echo "MAX_MTU_PAYLOAD=$SIZE" > $RESULTS_DIR/G_6_mtu_result.txt
        break
    else
        echo "FAIL: $SIZE"
    fi
...
kubectl exec $CLIENT_POD -- ip route show > $RESULTS_DIR/G_6_pod_routes.txt
kubectl exec $CLIENT_POD -- ip rule show > $RESULTS_DIR/G_6_pod_rules.txt
# Capture host routes (first worker)
NODE=$(kubectl get pod $CLIENT_POD -o jsonpath='{.spec.nodeName}')
docker exec $NODE ip route show > $RESULTS_DIR/G_6_host_routes.txt

# 4. TCP Stack Behavior (ss -ti)
echo "Capturing TCP Congestion Window (cwnd)..."
# Run iperf and ss in tandem
kubectl exec $CLIENT_POD -- /bin/sh -c "iperf3 -c $SVC_IP -t 5 & sleep 2; ss -ti" > $RESULTS_DIR/G_6_tcp_stats.txt
...
PCAP_FILE="/tmp/G_6_capture.pcap"
...
kubectl cp ${CLIENT_POD}:${PCAP_FILE} $RESULTS_DIR/G_6_capture.pcap
