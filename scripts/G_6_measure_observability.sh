#!/bin/bash
# scripts/G_6_measure_observability.sh - Network Observability Suite
...
if [ "$CNI" == "G_6_cilium" ]; then
    echo "Capturing Cilium Hubble Flows (eBPF)..."
    # Capture Hubble flows from the first G_6_cilium agent pod
    CILIUM_POD=$(kubectl -n kube-system get pods -l k8s-app=cilium -o jsonpath='{.items[0].metadata.name}')
    kubectl -n kube-system exec $CILIUM_POD -- cilium-dbg hubble observe --output json --last 100 > $RESULTS_DIR/G_6_hubble_flows.json || echo "Hubble capture failed"
    
    echo "Capturing Cilium Metrics (Prometheus format)..."
    kubectl -n kube-system exec $CILIUM_POD -- cilium-dbg metrics list > $RESULTS_DIR/G_6_cni_metrics.txt
fi
...
if [ "$CNI" == "calico" ]; then
    ...
    kubectl -n kube-system exec $CALICO_POD -- calico-node -version > $RESULTS_DIR/G_6_cni_version.txt
    # Capture IPPool status
    kubectl get ippools > $RESULTS_DIR/G_6_ippool_status.txt || echo "No IPPools found (Calico API not initialized)"
fi
