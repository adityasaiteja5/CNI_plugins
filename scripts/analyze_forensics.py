...
def run_suite(results_base):
    ...
    for cni in ['flannel', 'calico', 'cilium']:
        ...
        hops = parse_mtr(os.path.join(cni_dir, 'path_mtr.txt'))
        mtu = parse_mtu(os.path.join(cni_dir, 'mtu_result.txt'))
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python3 analyze_forensics.py <results_dir>")
        sys.exit(1)
