...
class DeepResearchEngine:
    def __init__(self):
        ...
        self.cni_dna = {
            ...
            "cilium":  {"throughput": 7, "latency": 10, "security": 10, "mttr": 5, "obs": 10}
        }
        self.scaling_slopes = {"flannel": 0.04, "calico": 0.02, "cilium": 0.0001}
    ...
    def simulate(self, config):
        scores = {"flannel": 0, "calico": 0, "cilium": 0}
        projections = {}
        insights = []
        ...
        for cni in scores:
            ...
            if policies > hw_limit and cni != "cilium":
                scores[cni] -= 40
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 predict_cni.py <scenario.json>")
        sys.exit(1)
