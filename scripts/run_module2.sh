...
for CNI in "${CNIS[@]}"; do
    ...
    RESULTS_BASE=$FINAL_RESULTS ./scripts/benchmark.sh $CNI --setup-only | tee $FINAL_RESULTS/logs/${CNI}_setup.log
    ...
    for COUNT in "${RULES[@]}"; do
        ...
        # 2. Apply Policies
        if [ $COUNT -gt 0 ]; then
            python3 scripts/generate_policies.py $COUNT > manifests/module2_policies.yaml
            kubectl apply -f manifests/module2_policies.yaml
        ...
        RESULTS_BASE="$FINAL_RESULTS/$CNI/rules_$COUNT" ./scripts/benchmark.sh $CNI --run-only | tee $FINAL_RESULTS/logs/${CNI}_rules_${COUNT}.log
        
        # 4. Cleanup Policies
        if [ $COUNT -gt 0 ]; then
            kubectl delete -f manifests/module2_policies.yaml
        fi
    done
...
python3 scripts/generate_plots.py $RESULTS_BASE
