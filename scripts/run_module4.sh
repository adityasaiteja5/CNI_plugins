#!/bin/bash
# scripts/run_module4.sh - End-to-End Network Path Analysis Orchestrator (Patched)
...
for CNI in "${CNIS[@]}"; do
    ...
    # Run Forensics
    ./scripts/measure_forensics.sh "$RESULTS_DIR/$CNI" "$CNI" "default" "$CLIENT_POD" "$SERVER_IP" "$SVC_IP"
...
python3 scripts/analyze_forensics.py $RESULTS_DIR
